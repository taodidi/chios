import chios, { ChiosRequestConfig, ChiosResponse } from '../src/index'
...
describe('instance', () => {
  ...
  test('should use instance options', () => {
    const instance = chios.create({ timeout: 1000 })
  ...
  test('should have defaults.headers', () => {
    const instance = chios.create({ baseURL: 'https://api.example.com' })
  ...
  test('should get the computed uri', () => {
    const fakeConfig: ChiosRequestConfig = {
    ...
    expect(chios.getUri(fakeConfig)).toBe(
